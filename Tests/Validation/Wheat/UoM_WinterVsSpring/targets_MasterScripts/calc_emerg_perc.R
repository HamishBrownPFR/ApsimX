...
#' @param consider_blank_as_zero Logical or Character. If TRUE or "Yes", converts NA/blank values to 0 before calculation. Defaults to FALSE.
#'
#' @return The original tibble with a newly appended row containing the percentage data.
#' @export
calc_emerg_perc <- function(df_tbl, df_input_var_name, df_new_var_name, consider_blank_as_zero = FALSE) {
  ...
  # ---- 2.5 SILENT NA-TO-ZERO CONVERSION ----
  na_converted_count <- 0
  if (isTRUE(consider_blank_as_zero) || identical(toupper(consider_blank_as_zero), "YES")) {
    na_converted_count <- sum(is.na(df_target[[val_col]]))
    if (na_converted_count > 0) {
      df_target <- df_target %>%
        dplyr::mutate(!!val_col := tidyr::replace_na(.data[[val_col]], 0))
    }
  }
  ...
  if (na_converted_count > 0) {
    log_box <- c(log_box, sprintf(" -> Data Repair   : Converted %d blank/NA records to 0.", na_converted_count))
  }
