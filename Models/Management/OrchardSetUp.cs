...
namespace Models.Management
{
    ...
    public class OrchardSetUp : Model, IStructureDependency
    {
        ...
        {
            {"SpringDormancy","[Apple].Phenology.SpringDormancy.DateToProgress = " },
            {"CanopyExpansion","[Apple].Phenology.CanopyExpansion.DateToProgress = " },
            {"FullCanopy","[Apple].Phenology.FullCanopy.DateToProgress = " },
            {"LeafFall", "[Apple].Phenology.LeafFall.DateToProgress = " },
            {"WinterDormant", "[Apple].Phenology.WinterDormancy.DateToProgress = " },
            {"MaxRootDepth","[Apple].Root.Network.MaximumRootDepth.FixedValue = "},
            {"Proot","[Apple].Root.TotalCarbonDemand.TotalDMDemand.PartitionFraction.FixedValue = " },
            {"Pleaf","[Apple].Leaf.TotalCarbonDemand.TotalDMDemand.PartitionFraction.FixedValue = " },
            {"Ptrunk","[Apple].Trunk.TotalCarbonDemand.TotalDMDemand.Available.PartitionFraction.FixedValue = " },
            {"MaxPrunedHeight","[Apple].MaxPrunedHeight.FixedValue = " },
            {"CanopyBaseHeight","[Apple].Height.CanopyBaseHeight.Maximum.FixedValue = " },
            {"MaxSeasonalHeight","[Apple].Height.SeasonalGrowth.Maximum.FixedValue = " },
            {"MaxPrunedWidth","[Apple].Width.PrunedWidth.Maximum.FixedValue = "},
            {"MaxSeasonalWidth","[Apple].Width.SeasonalGrowth.Maximum.FixedValue = " },
            {"ProductNConc","[Apple].Fruit.Nitrogen.ConcFunctions.Maximum.FixedValue = "},
            {"ResidueNConc","[Apple].Leaf.Nitrogen.ConcFunctions.Maximum.FixedValue = "},
            {"RootNConc","[Apple].Root.Nitrogen.ConcFunctions.Maximum.FixedValue = "},
            {"WoodNConc","[Apple].Trunk.Nitrogen.ConcFunctions.Maximum.FixedValue = "},
            {"ExtinctCoeff","[Apple].Leaf.Canopy.GreenExtinctionCoefficient.UnstressedCoeff.FixedValue = "},
            {"BaseLAI","[Apple].Leaf.Canopy.GreenAreaIndex.Winter.BaseArea.FixedValue = " },
            {"AnnualDeltaLAI","[Apple].Leaf.Canopy.GreenAreaIndex.SeasonalGrowth.AnnualDelta.FixedValue = " },
            {"DecidiousSenescence","[Apple].Leaf.SenescenceRate.DecidiousSensecence.LeafFall.MultiplyFunction.Switch.FixedValue = " },
            {"EverGreenSenescence", "[Apple].Leaf.SenescenceRate.EvergreenSenescence.Coefficient.FixedValue = "},
            {"GSMax","[Apple].Leaf.Canopy.Gsmax350 = " },
            {"R50","[Apple].Leaf.Canopy.R50 = " },
            {"SurfaceKL","[Apple].Root.Network.KLModifier.SurfaceKL.FixedValue = " },
            {"RUE","[Apple].Leaf.Photosynthesis.RUE.FixedValue = " },
            {"PSBaseT","[Apple].Leaf.Photosynthesis.FT.XYPairs.X[1] = " },
            {"PSLOptT","[Apple].Leaf.Photosynthesis.FT.XYPairs.X[2] = " },
            {"PSUOptT","[Apple].Leaf.Photosynthesis.FT.XYPairs.X[3] = " },
            {"PSMaxT","[Apple].Leaf.Photosynthesis.FT.XYPairs.X[4] = " },
            {"FRGRUOptT", "[Apple].Leaf.Canopy.FRGRer.FT.XYPairs.X[3] = " },
            {"FRGRMaxT", "[Apple].Leaf.Canopy.FRGRer.FT.XYPairs.X[4] = " },
            {"FRGRMaxTY", "[Apple].Leaf.Canopy.FRGRer.FT.XYPairs.Y[4] = " },
            {"InitialTrunkWt","[Apple].Trunk.InitialWt.FixedValue = "},
            {"InitialRootWt", "[Apple].Root.InitialWt.FixedValue = " },
            {"InitialFruitWt","[Apple].Fruit.InitialWt.FixedValue = "},
            {"InitialLeafWt", "[Apple].Leaf.InitialWt.FixedValue = " },
            {"YearsToMaturity","[Apple].RelativeAnnualDimension.XYPairs.X[2] = " },
            {"TrunkWtAtMaturity","[Apple].Trunk.MatureWt.FixedValue = " },
            {"YearsToMaxRD","[Apple].Root.Network.RootFrontVelocity.RootGrowthDuration.YearsToMaxDepth.FixedValue = " },
            {"Number","[Apple].Fruit.Number.RetainedPostThinning.FixedValue = " },
            {"FruitDensity","[Apple].Fruit.Density.FixedValue = " },
            {"DryMatterContent", "[Apple].Fruit.DMC.FixedValue = " },
            {"DateMaxBloom","[Apple].Phenology.DaysSinceFlowering.StartDate = "},
            {"DateResetFloweringDAS","[Apple].Phenology.DaysSinceFlowering.ReduceDates = "},
            {"DAFStartLinearGrowth","[Apple].Fruit.TotalCarbonDemand.RelativeFruitMass.Delta.Integral.XYPairs.X[2] = "},
            {"DAFEndLinearGrowth","[Apple].Fruit.TotalCarbonDemand.RelativeFruitMass.Delta.Integral.XYPairs.X[3] = "},
            {"DAFMaxSize","[Apple].Fruit.TotalCarbonDemand.RelativeFruitMass.Delta.Integral.XYPairs.X[4] = "},
            {"WaterStressPhoto","[Apple].Leaf.Photosynthesis.Fw.XYPairs.Y[1] = "},
            {"WaterStressPhoto2","[Apple].Leaf.Photosynthesis.Fw.XYPairs.Y[2] = "},
            {"WaterStressExtinct","[Apple].Leaf.Canopy.GreenExtinctionCoefficient.WaterStressFactor.XYPairs.Y[1] = "},
            {"WaterStressNUptake","[Apple].Root.Network.NUptakeSWFactor.XYPairs.Y[1] = "},
            {"PotentialFWPerFruit","[Apple].Fruit.PotentialFWPerFruit.FixedValue = " },
            {"RowWidth","[Apple].RowWidth.FixedValue = " },
            {"InterRowSpacing","[Apple].InterRowSpacing.FixedValue = " }
        };
        ...
        public void Establish()
        {
            ...
            var soilCrop = Structure.FindChild<SoilCrop>(treeModel.Name + "Soil", relativeTo: soil, recurse: true);
            var physical = Structure.FindChild<Physical>("Physical", relativeTo: soil, recurse: true);
            if (soilCrop == null)
                throw new Exception($"Cannot find a soil crop parameterisation called {treeModel.Name}Soil");
            ...
            // Apple sets soil KL to 1 and uses the KL modifier to determine appropriate kl based on root depth
            ...
            tree = CoeffCalc();
            treeModel.AddCultivar(tree);
            treeModel.Sow(cropName, population, depth, rowWidth);
            phenology.SetAge(AgeAtSimulationStart);
            summary.WriteMessage(this,"Some of the message above is not relevent as Apple has no notion of population, bud number or row spacing." +
                " Additional info that may be useful.  " + this.Name + " is established as " + this.AgeAtSimulationStart.ToString() + " Year old plant "
                ,MessageType.Information);
        }

        /// <summary>
        /// Data structure that holds Apple parameter names and the cultivar overwrite they map to
        /// </summary>
