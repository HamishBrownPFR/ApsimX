...
branches = {#"master":"master",#"master", 
            "dev":"Lentil", 
            #"working":"FittingWheat"
            }   # branches to test
            
simulation_files = [
    r'C:\GitHubRepos\ApsimX\Prototypes\Lentil\Lentil.apsimx',
    r'C:\GitHubRepos\ApsimX\Prototypes\Lentil\NaPA\NaPA.apsimx'
]
...
def makeLists(HarvestPred, branches):
    ...
    for branch in branches.keys():
        ...
        Cultivars[branch]   = unique_sorted(HarvestPred[branch]['Lentil.SowingData.Cultivar'])
    ...
    return Latitudes, Longitudes, Cultivars, Experiments, Folders, Files

Latitudes, Longitudes, Cultivars, Experiments, Folders, Files = makeLists(HarvestPred, branches)
...
HarvestObs = {}
for branch in branches.keys():
    HarvestObs[branch] = Observed[branch].loc[Observed[branch].loc[:,'Lentil.Phenology.CurrentStageName']=='HarvestRipe',:].dropna(axis=1,how='all').dropna(axis=0,how='all')
...
#     """
#     Return the most frequent non-blank/non-NaN value in x; np.nan if none.
#     - Treats categorical and object uniformly as strings for blank detection.
#     - Preserves original values for non-string types.
#     """
#     # Drop actual NaNs first
#     s = x.dropna()

#     if s.empty:
#         return np.nan

#     # If dtype is object/category, remove empty/whitespace strings safely
#     if pd.api.types.is_object_dtype(s) or pd.api.types.is_categorical_dtype(s):
#         # Convert to string for blank detection
#         s_str = s.astype(str)
#         mask_non_blank = s_str.str.strip() != ""
#         s = s[mask_non_blank]
#         if s.empty:
#             return np.nan

#     # For all dtypes, value_counts handles mode; keep original values (not categorical codes)
...
#     to unique rows by aggregating:
#       - numeric cols => mean
#       - non-numeric cols => mode_non_blank
#       - datetime cols => first non-NaN
#     Optionally force dtypes for measures/labels before aggregation.
#     Returns a DataFrame indexed by (File, SimulationName, Clock.Today).
#     """

#     # Ensure index has the right levels; if not, try to set from columns
#     needed = ['File', 'SimulationName', 'Clock.Today']
#     if (df.index.nlevels < 3) or (list(df.index.names[:3]) != needed):
#         idx_cols = [c for c in needed if c in df.columns]
#         if len(idx_cols) == 3:
#             df = df.set_index(idx_cols)
...
#     # (Optional) enforce intended dtypes to prevent category leaks into measures
#     if measure_cols:
#         for c in measure_cols:
#             if c in df.columns:
#                 # If categorical or object with numeric-like content, coerce to numeric
#                 if (pd.api.types.is_categorical_dtype(df[c]) or
...
#         for c in label_cols:
#             if c in df.columns:
#                 # Convert to category (memory friendly) but won't be used in math
#                 if not pd.api.types.is_categorical_dtype(df[c]):
#                     df[c] = df[c].astype("category")

#     # Build aggregation dict for dtypes
#     num_cols   = df.select_dtypes(include=['number']).columns.tolist()
#     dt_cols    = df.select_dtypes(include=['datetime']).columns.tolist()
#     non_cols   = [c for c in df.columns if c not in num_cols + dt_cols]
...
#     # Datetime => first non-NaN
#     for c in dt_cols:
#         agg_dict[c] = 'first'

#     # Non-numeric (object/category/others) => mode_non_blank
#     for c in non_cols:
#         agg_dict[c] = mode_non_blank
...
# def _ensure_unique_index(df: pd.DataFrame, keep_cols: list[str]) -> pd.DataFrame:
#     """
#     Ensure df has a unique MultiIndex by grouping on all index levels and taking the first row.
#     keep_cols: columns to keep in the grouped result
#     """
